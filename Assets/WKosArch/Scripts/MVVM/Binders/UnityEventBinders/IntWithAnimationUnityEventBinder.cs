...
{
    public class IntWithAnimationUnityEventBinder : UnityEventBinder<int>
    {
        [SerializeField]
        ...
        {
            int initialValue = int.Parse(_pointTMPro.text);
            DOTween.Kill(_pointTMPro); // Kill previous tweens on this object to avoid overlaps

            Sequence sequence = DOTween.Sequence();
            sequence.Append(DOTween.To(() => initialValue, x => _pointTMPro.text = x.ToString(), newValue, _animationDuration)
                        .SetEase(Ease.OutQuad)) // Controls the pace of the score counting
